...
class UserBloc {
  ...
  final _existingAuthController = BehaviorSubject<String>.seeded(null);
  Stream<String> get existingAuthId => _existingAuthController.stream; 

  Stream<UserAccountStatus> _accountStatusController;
  ...
  Stream<bool> get noUserFound => _noUserFoundController.stream;

  final _loadingController = PublishSubject<bool>();
  Stream<bool> get isLoading => _loadingController.stream;
  final _isLoadingFollowsController = PublishSubject<bool>();
  Stream<bool> get isLoadingFollows => _isLoadingFollowsController.stream;
  ...
  Stream<String> get verificationEmail => _verificationEmailController;
  ...
  Stream<bool> get isUsernameTaken => _isUsernameTakenController;
  ...
  UserBloc(this._repository, this._outfitRepository, this._preferences, this._outfitBloc) {
    _subscriptions = <StreamSubscription<dynamic>>[
      ...
      _checkUsernameController.stream.debounce((_) => TimerStream(true, Duration(milliseconds: 500))).listen(_refreshUsernameCheck),
    ...
    _accountStatusController = Rx.combineLatest3<String, User, bool, UserAccountStatus>(existingAuthId, _currentUserController, _loadingController, _redirectPath).asBroadcastStream().debounce((_) => TimerStream(true, Duration(milliseconds: 300)));
