...
class CachedOutfitRepository {
  ...
  Future<int> addOutfitSearch(int outfitId, SearchModes searchMode) async {
    ...
    //TODO: trigger 'outfit_search','outfit'
    return streamDatabase.rawInsert("INSERT INTO outfit_search (search_outfit_id, search_outfit_mode) VALUES (?, ?)", [outfitId, searchModeString]);
  }
  ...
  Future<void> updateComment(AddComment addComment, int tempCommentId, int actualCommentId) async { 
    await streamDatabase.rawUpdateAndTrigger(['comment'], 'UPDATE comment SET comment_id=? WHERE comment_id=?', [actualCommentId, tempCommentId]);
