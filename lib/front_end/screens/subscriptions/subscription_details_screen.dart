...
class _SubscriptionDetailsScreenState extends State<SubscriptionDetailsScreen> {
  ...
  void asyncInitState() async {
    try {
      ...
      await FlutterInappPurchase.instance.initConnection
          .then((res) => print('connection inited: $res'));
      isSubscribed =
  ...
  _checkConnection() async {
    ...
    if (!isConnectedNow) {
      throw PlatformException(code: 'NO CONNECTION!');
    }
  }

  _getItems() async {
    List<IAPItem> items = await FlutterInappPurchase.instance
        .getSubscriptions(AdmobTools.subscriptionId);
  ...
  _showLogs() {
    ...
    subscriptionItem
        .toString()
        .split(',')
        .forEach((entry) => productLog += '$entry\n\n');
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          Expanded(flex: 3, child: _benefitsOverview()),
          Expanded(
            flex: 2,
      ...
      child: CarouselSliderWithIndicator(
        ...
        items: <Widget>[
          BenefitOverview(
            ...
            description:
                'Go wild experimenting with new fits all day long with no limits!',
          ),
          BenefitOverview(
            icon: FontAwesomeIcons.ad,
            title: 'No ads',
            description:
                'Spend as much time as you want browsing and interacting with outfits with no interruptions!',
          ),
          BenefitOverview(
            icon: FontAwesomeIcons.server,
            title: 'Unlimited lookbooks storage',
            description:
                'Enjoy unlimited number of outfits across all your customized lookbooks for every style',
          ),
          BenefitOverview(
            icon: FontAwesomeIcons.globeEurope,
            title: 'Custom country search',
            description:
                'See the different & unique fashion trends from any country in the world!',
          ),
          BenefitOverview(
            icon: FontAwesomeIcons.calendarAlt,
            title: 'Custom date range search',
            description:
                "Want to see the hottest fits from new year's day? Halloween? or even in the middle of the summer?\nSearch for the best outfits across any date range!",
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
        isLoading || !hasConnection
            ? Container()
            : (hasSubscription ? _paidBanner() : _itemPrice()),
        _buyButton(),
        _legalInfo(),
        _restoreButton(),
      ]),
  ...
  Widget _buyButton() {
    return Container(
      ...
      child: RaisedButton(
        onPressed: hasSubscription || isLoading || !hasConnection
            ? null
            : _unlockSubscription,
        color: Colors.blue,
        padding: EdgeInsets.all(16),
  ...
  _unlockSubscription() async {
    try {
      await FlutterInappPurchase.instance
          .requestPurchase(subscriptionItem.productId);
  ...
  _switchSubscription(bool isActive) {
    ...
    _preferences.updatePreference(
        Preferences.HAS_SUBSCRIPTION_ACTIVE, isActive);
          ...
          children: [
            TextSpan(
              ...
              recognizer: TapGestureRecognizer()
                ..onTap =
                    () => UrlLauncher.openURL(AppConfig.PRIVACY_POLICY_URL),
            ),
            TextSpan(
                text: ' and ',
                style: TextStyle(
            ...
            TextSpan(
              ...
              recognizer: TapGestureRecognizer()
                ..onTap = () =>
                    UrlLauncher.openURL(AppConfig.TERMS_AND_CONDITIONS_URL),
            ),
          ]),
    );
  }
  ...
  _restorePurchases() async {
    ...
    isSubscribed = await FlutterInappPurchase.instance
